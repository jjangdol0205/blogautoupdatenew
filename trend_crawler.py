...
def extract_golden_keywords_with_gemini(daum_headlines, nate_stories):
    ...
    [9가지 필수 카테고리]
    1. 정치 우파 관점 (보수, 안보, 자유시장경제, 우파적 불안감 자극)
    2. 정치 좌파 관점 (진보, 복지, 노동권, 좌파적 불평등/민생 자극)
    3. 보조금/지원금/복지 (미신청 시 손실 공포 자극)
    4. 예적금/특판 (마감 임박, 기회비용 공포 자극)
    5. 연금/시니어 (국민연금 개혁, 건보료 폭탄 방어)
    6. 부동산/청약 (로또 줍줍, 벼락거지 공포 자극)
    7. 세금/절세 (세금 신고 누락 가산세 공포 자극)
    8. 도파민/이슈 (사회적 논란, 연예인, 가십 관음증 자극)
    9. 네이트판 인기 스토리 (커뮤니티 레전드 썰, 고부갈등, 인간관계 도파민 자극)
    
    [출력 형식]
    각 카테고리별로 가장 파급력이 클 것 같은 "정제된 키워드(또는 제목)"를 1~2개씩만 뽑아주세요.
    반드시 다음과 같이 "정제된 키워드 | 카테고리명" 형식으로 한 줄씩 출력해주세요. 다른 부연 설명은 절대 적지 마세요.
    (예시: 2026 숨은 정부지원금, 안받으면 매달 50만원 손해 | 보조금/지원금/복지)
