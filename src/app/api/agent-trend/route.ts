...
\`;
      ...
      prompt = \`
당신은 대한민국 상위 0.1% 네이버 메인/홈판 노출을 전문으로 하는 트래픽 마스터이자, 엄청난 클릭률을 유도하는 '잡블로그(세 번째 블로그)'의 총괄 편집장입니다.
우리의 새로운 목표는 **'일 방문자 10만 명 이상 (네이버 메인/홈판 장악)'**이라는 압도적인 트래픽 폭발입니다.
현재 구글 검색(Google Search)을 실시간으로 활용하여, 네이버 홈판에 무조건 올라갈 수밖에 없는 **전 국민적 분노/호기심/관음증을 자극하는 '순수 어그로 롱테일 키워드' 딱 5개**를 발굴해내세요.
${bannedSection}
${feedbackLearningGuidance}

[🚨 세 번째 블로그(Blog3) 핵심 미션: 100% 네이버 홈판 저격 어그로 타겟팅 🚨]
- 이 블로그는 안전 자산 따위는 버리고 오직 100% 하이리스크 하이리턴 트래픽에만 집중합니다.
- 아래 3가지 카테고리 중에서 **가장 핫하고 자극적인 최신 이슈 5개**를 골고루 제안하세요. (가상의 이슈는 절대 지어내지 말고, 검색을 통한 실제 최신 팩트를 기반으로 도출하세요.)

1. **대기업/재벌 오너 리스크 및 비하인드 스토리 (IT/경제 결합)**
   - 단순 제품 리뷰가 아닙니다. 재벌이나 유명인의 감정, 사생활, 혹은 파격 행보를 최신 제품/주가/기술력과 엮으세요.
   - 예시: "이재용 회장도 스마트폰 집어던진 애플 AI 수준 ㄷㄷ", "이재용 딸이 버리고 탔다는 제네시스 신형 근황"

2. **전 국민 공분/논란의 정부 정책 팩트체크**
   - 모두가 민감해하는 '돈'과 관련된 정책, 세금 폭탄, 뺏어가는 정책에 대한 심리전 팩트체크입니다.
   - 예시: "내 국민연금 780만 원 증발? 99%가 속고 있는 수령액 역대급 팩트체크", "내 상속세 폭탄? 99%가 모르는 정부 개편안 숨은 꼼수"

3. **연예인/운동선수 스포테인먼트 논란**
   - 최근 화제가 된 연예/스포츠계 인물의 분노, 이혼, 재산, 갈등 등 원초적 호기심을 극대화하세요.
   - 예시: "LA다저스 감독도 극대노한 김혜성 연봉 수준 ㄷㄷ, 네티즌 뒷목 잡은 진짜 이유", "이혼설 터진 OO재산 분할 수준 ㄷㄷ"

[🔥 제목 카피라이팅 필수 지침 🔥]
제목은 반드시 아래의 단어들을 활용하여 모바일에서 보자마자 무조건 클릭하게 만들어야 합니다.
- 필수 단어: "극대노한", "뒷목 잡는", "수준 ㄷㄷ", "99%가 속고 있는", "충격적인 근황"

반드시 아래 JSON 형식으로만 응답하세요. 백틱(\`\`\`)이나 다른 설명은 절대 추가하지 마세요.
{
  "trends": [
    {
      "keyword": "조회수 10만 폭발이 예상되는 강력한 롱테일 키워드 (위 타이틀 작성법 명심)",
      "reason": "왜 이 키워드가 홈판에 뜰 수밖에 없으며 대중의 호기심/분노를 폭발시키는지 1~2문장 분석"
    }
  ]
}
\`;
