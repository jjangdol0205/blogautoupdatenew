...
export async function POST(req: Request) {
  try {
    ...
    let keywordGuidance = "";
    if (blogType === 'health') {
      keywordGuidance = "건강/라이프 블로그용이므로, 의학적/상징적 개념보다는 '신선한 과일', '채소', '자연 풍경', '따뜻한 차', '산책로' 등 시각적으로 거부감 없고 따뜻한 사물/자연의 '한글 혹은 영어 단어' 1~2개를 명사형태로 선택하세요. 서양인들이 가득한 본문 사진은 피하세요.";
    } else if (blogType === 'trot') {
      keywordGuidance = "트로트 팬덤 블로그용이므로 절대로 '가수', '사람', '인물' 관련 단어를 뽑지 마세요! 픽사베이에는 서양인 가수만 나옵니다. 무조건 '마이크', '무대 조명', '음표', '꽃다발', '반짝이는 무대', '야광봉' 등 음악과 감동을 상징하는 사람이 없는 감성적인 사물 '한글 단어'만 명사형태로 선택하세요.";
