...
독자가 어색함을 느끼지 못하도록 진짜 정보인 것처럼 녹여내야 최적화 블로그 점수를 받습니다.`;
    ...
    3. englishSubject: 이 주제를 그림으로 그릴 때 메인 피사체가 될 만한 구체적인 영단어 2~3개 (예: "smartphone music", "stock chart analysis", "korean food")
    
    예시:
    "밸류업 관련주 추천" -> {"primary": "주식 차트", "fallback": "금융", "englishSubject": "stock market chart rising"}
    "삼성전자 주가방향" -> {"primary": "동전 지갑", "fallback": "비즈니스", "englishSubject": "finance business technology"}
    "음악어플가격비교" -> {"primary": "음악 듣기", "fallback": "스마트폰", "englishSubject": "listening to music on smartphone"}

    반드시 아래 JSON 형식으로만 응답하세요. 다른 문장 부호나 설명은 절대 붙이지 마세요.
    {"primary": "...", "fallback": "...", "englishSubject": "..."}
    ...
    let searchParams = { primary: "사무실", fallback: "비즈니스", englishSubject: "office desktop" };
    try {
...
1. 도입부 (FOMO 자극 및 공감):
   - 글 시작 부분에서 독자의 불안감이나 결핍(FOMO)을 강력하게 자극하세요. 예: "혹시 지금 'OOO' 검색해보고 불안한 마음에 클릭하셨나요?", "나만 또 소외될까 봐 조급한 마음, 제가 다 압니다."
   - 이 글 하나만 읽으면 금전적 손해를 막고, 독점적인 꿀팁을 얻어갈 수 있다는 것을 명시하며 체류시간을 극대화하세요.

2. 페르소나 (전문가 + 친근한 이웃):
   - 독자를 "여러분" 혹은 "이웃님들"로 친근하게 부르며, "제가 다 압니다", "정신 바짝 차리고 따라오세요!" 같은 구어체(~해요, ~입니다)를 사용하세요.
   - 정보는 최고 전문가 수준으로 깊이 있고 정확하게 전달하여 신뢰감을 줍니다.

3. 본문 구조 (목차형 구조 및 시각적 기호 필수):
   - 긴 글은 반드시 짧게 쪼개어 가독성을 높입니다.
   - 소제목에는 반드시 시각적 기호(📌, ✅, ✔️, 💡 등)를 사용하고, 번호를 매기세요. 예: "📌 [1. 그래서, 누가 받을 수 있나요?]", "✅ 숨겨진 조건 총정리"
   - 구체적인 정보(예: 소득 연 몇 만원 이하, 건강보험료 기준, 신청 사이트 '복지로', '정부24' 등 구체적 기관명)를 반드시 포함하여 작성하세요.

4. 결론 (핵심 요약 및 행동 촉구 - CTA):
   - 포스팅 맨 마지막에는 반드시 "✨ 오늘 내용 핵심 요약 정리! ✨"라는 소제목으로 가장 중요한 정보 3~5가지를 다시 요약하세요.
   - 독자가 지금 당장 조치할 수 있도록 행동을 촉구(예: "지금 바로 신청하세요!", "궁금한 건 댓글 남겨주세요!")하며 포스팅을 마무리하세요.
...
`;
    ...
    // 2. [비주얼 강화] 구글 최신 Imagen 4 모델을 활용한 유튜브 스타일 초고화질 썸네일 생성
    let thumbnailHtml = "";
    try {
      // 텍스트에서 추출한 제목 중 핵심 단어만 축약해서 이미지에 넣기 (가독성 향상)
      const shortTitle = parsedResult.title.length > 18 ? parsedResult.title.substring(0, 18) : parsedResult.title;
      
      const imgRes = await ai.models.generateImages({
        model: 'imagen-4.0-fast-generate-001',
        prompt: `A vibrant, highly detailed 2D anime style YouTube thumbnail about "${searchParams.englishSubject}". Eye-catching vibrant colors, professional layout. In the center, clearly display the text "${shortTitle}". The scene must clearly show people or objects related to the subject. No abstract empty gradients.`,
        config: { 
          numberOfImages: 1, 
          aspectRatio: '16:9',
      ...
      if (base64Image) {
        thumbnailHtml = `<div style="text-align: center; margin-bottom: 24px;">
          <img src="data:image/jpeg;base64,${base64Image}" alt="대표 썸네일" style="max-width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05);" />
        </div>`;
      } else {
        throw new Error("No image generated by Imagen API");
      }
    } catch (imgError) {
      console.error("Imagen 4 Thumbnail Generation Failed, falling back to basic OG image:", imgError);
